class TrainingsController < ApplicationController
  ...
  def index_upcoming
    ...
    # Index for Sevener Users, with limited visibility
      ...
      @trainings = Training.joins(sessions: :users).where("users.email LIKE ?", "#{current_user.email}").uniq.select{|x| Session.joins(:session_trainers).where(training_id: x.id, session_trainers: {user_id: current_user.id}).order(date: :asc).last.date > Date.today}.sort_by{|y| y.next_session}
  ...
  def index_completed
    ...
    # Index for Sevener Users, with limited visibility
      ...
      @trainings = Training.joins(sessions: :users).where("users.email LIKE ?", "#{current_user.email}").uniq.select{|x| Session.joins(:session_trainers).where(training_id: x.id, session_trainers: {user_id: current_user.id}).order(date: :asc).last.date <= Date.today}.sort_by{|y| y.end_time}.reverse
    end
    skip_authorization
